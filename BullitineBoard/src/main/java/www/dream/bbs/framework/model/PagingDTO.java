...
public class PagingDTO {
	...
	private static final int BTN_CNT = 3;
	private int page;
	private long offset;
	...
	// 실수형 화면 구성, 현재 쪽 기준으로 앞 뒤 맞추기, 중앙 배치
	...
	public void buildPagination(long dataCount) {
		// 전체 쪽 수
		int toPageCount = (int) Math.ceil((float) dataCount / LIMIT);
		// 전체 쪽 수의 최소값
		toPageCount = toPageCount < 1 ? 1 : toPageCount;
		// 마지막 쪽 번호의 기본값 산출
		lastPage = page + BTN_CNT / 2;
		// 뒤에 표현할 개수 모자라면 눌러주자
		if (lastPage < BTN_CNT) lastPage = BTN_CNT;
		// 가상적인 마지막이 실제 마지막보다 크면 이동 조정
		if (lastPage > toPageCount) lastPage = toPageCount;
		startPage = lastPage - BTN_CNT + 1;
		if (startPage < 1) startPage = 1;
		prev = startPage > 1;
		next = lastPage < toPageCount;
	}
}
